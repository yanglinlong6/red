...
natives: context [
	...
	do*: func [
		...
		if next > 0 [slot: _context/get as red-word! stack/arguments + next]
			...
			switch TYPE_OF(arg) [
				...
				TYPE_STRING [
					lexer/load-string arg as red-string! arg -1 no yes yes no null null as red-string! arg
					DO_EVAL_BLOCK
				]
				TYPE_URL 
	...
	transcode*: func [
		...
		either type = TYPE_BINARY [
			...
			type: lexer/load-string slot str len one? scan? load? no :offset fun as red-series! str
		]
